...
<body class="bg-slate-50 text-gray-900">
    ...
    <script>
        ...
        function renderCategoryOptions(subCategoryId) {
            categorySelect.innerHTML = '';
            facilitySelect.innerHTML = '<option value="">-- Pilih Kategori terlebih dahulu --</option>';
            facilitySelect.disabled = true;

            if (!subCategoryId) {
                categorySelect.disabled = true;
                categorySelect.innerHTML = '<option value="">-- Pilih Lokasi / Gedung terlebih dahulu --</option>';
                return;
            }

            // hanya kategori yang punya fasilitas di gedung ini
            const availableCategoryIds = [...new Set(
                allFacilities
                .filter((facility) => String(facility.subcategory_id) === String(subCategoryId))
                .map((facility) => String(facility.category_id))
            )];

            const filteredCategories = allCategories.filter(
                (category) => availableCategoryIds.includes(String(category.id))
            );

            if (filteredCategories.length === 0) {
                categorySelect.disabled = true;
                categorySelect.innerHTML = '<option value="">-- Tidak ada kategori tersedia di lokasi ini --</option>';
                return;
            }
            ...
            filteredCategories.forEach((category) => {
                ...
                if (oldCategoryId && String(oldCategoryId) === String(category.id)) {
        ...
        function renderFacilityOptions(subCategoryId, categoryId) {
            ...
            const filtered = allFacilities.filter(
                (facility) =>
                String(facility.subcategory_id) === String(subCategoryId) &&
                String(facility.category_id) === String(categoryId)
            );

            if (filtered.length === 0) {
                facilitySelect.disabled = true;
                facilitySelect.innerHTML = '<option value="">-- Tidak ada fasilitas tersedia --</option>';
                return;
            }
        ...
        categorySelect.addEventListener('change', function() {
            renderFacilityOptions(subCategorySelect.value, this.value);
        });

        document.addEventListener('DOMContentLoaded', function() {
            if (subCategorySelect.value) {
                renderCategoryOptions(subCategorySelect.value);
                if (categorySelect.value) {
                    renderFacilityOptions(subCategorySelect.value, categorySelect.value);
                }
            }
        });
    </script>
